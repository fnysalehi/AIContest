...
class DefensiveReflexAgent(ReflexCaptureAgent):
    ...
    def getFeatures(self, gameState, action):
            ...
            if (17, 7) not in gameState.getWalls().asList():
                self.s = (17, 7)
            elif (16, 7) not in gameState.getWalls().asList():
                self.s = (16, 7)
            elif (18, 7) not in gameState.getWalls().asList():
                self.s = (18, 7)
            elif (15, 7) not in gameState.getWalls().asList():
                self.s = (15, 7)
            elif (19, 7) not in gameState.getWalls().asList():
                self.s = (19, 7)
    ...
    def chooseAction(self, gameState):
        ...
        if not gameState.getAgentState(self.index).isPacman:
            ...
            if len(nearestEnemy) > 0:
                if nearestEnemy[1] < 4 :
                    if red :
                            self.escapepath = self.bfs(gameState, self.width, self.height, nearestEnemy, [self.width / 2 - 5])
                    else :
                        self.escapepath = self.bfs(gameState, self.width, self.height, nearestEnemy,
                                                   [self.width / 2 + 5])
            else:
                self.escapepath = []
                if self.eaten >= 1:
                    if red :
                        self.escapepath = self.bfs(gameState, 36, 17, nearestEnemy, [self.width / 2 - 5])
                    else :
                        self.escapepath = self.bfs(gameState, 36, 17, nearestEnemy, [self.width / 2 + 5])
